...
impl BotGame{
    pub fn new_from_challenge(game_info: &GameEventInfo) -> BotGame{
        ...
        let board = chess::Board::from_str(&*game_info.fen).expect("Board could not be made from fen");
    ...
    pub fn enter_opponent_move(&mut self, move_chain: &str) -> (){
        ...
        if current_board != online_board{    
            ...
            let last_opponent_move = move_chain.split(" ").last().expect(&format!("No last move found in {}", move_chain));
            let chess_move = chess::ChessMove::from_str(last_opponent_move).expect(&format!("Unable to gather last opponent move {last_opponent_move} from chain: {move_chain}"));
            // Assert opponent's move legality, panics if not the case
            assert!(current_board.legal(chess_move), "Move {:?} not in legal moves {:?}", chess_move, moves_iterator.map(|x| x.to_string()).collect::<Vec<String>>());
...
pub fn yield_next_move(bot_game: Arc<Mutex<crate::game::BotGame>>, mut api: Arc<ApiHandler>) -> (String, bool) {
    debug!("Yield next move called");
    let next_move_receiver = api.pool.schedule_job(move || _yield_next_move(bot_game.clone()));
