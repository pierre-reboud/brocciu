...
impl Searcher{    
    ...
    fn _get_next_move_random(&mut self) -> Result<(String, bool), NoAvailableMoveError> {
        ...
        let str_move = match moves.choose(&mut rand::thread_rng()){
            Some(mv) => {
                ...
                (*self.game).borrow_mut().current_position().make_move_new(mv.clone());
    ...
    fn _get_next_move_mcts(&mut self, my_color: chess::Color, ) -> Result<(String, bool), NoAvailableMoveError> {
